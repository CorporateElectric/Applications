...
var ServiceModule = function () {
    ...
    return {
        ...
        addonserviceTemplate: function (val, config, template, edit, configTxt, layout, extra_class) {
            var section = '';
            ...
            if (edit == 'N') {
                ...
                section += '<i class="action-icon delete fa fa-trash"></i>';
                section += '</a>';
                section += '<a href="javascript:;" data-filter="' + template + '" title="Edit" data-id="' + iCount + '" class="add-on-service">';
                section += '<i class="action-icon edit fa fa-pencil"></i>';
                section += '</a>';
                section += '<div class="clearfix"></div>';
                section += '<div class="defoult-module section-item addonserviceTemplate" data-editor="' + iCount + '">';
                section += '<div class="col-md-12"><label><b>' + val + '</b></label><ul><li>Template: ' + template + '</li></ul></div>';
                section += '<input data-extclass="' + extra_class + '" id="' + iCount + '" data-layout="' + layout + '" data-type="' + template + '" data-config="' + config + '" type="hidden" class="txtip" value="' + val + '"/>';
                section += '<div class="clearfix"></div>';
                section += '</div>';
        ...
        // 24-11-2021 Add On Services Start Code
        submitFrmAddonSectionServiceModuleTemplate: function () {
            if ($('#frmSectionAddonServiceModuleTemplate').validate().form()) {
                var edit = $('#frmSectionAddonServiceModuleTemplate input[name=editing]').val() != '' ? $('#frmSectionAddonServiceModuleTemplate input[name=editing]').val() : 'N';
                $('#no-content').addClass('hide');
                $('#has-content').removeClass('hide');
                var val = $('#frmSectionAddonServiceModuleTemplate input[name=section_title]').val();
                var extra_class = $('#frmSectionAddonServiceModuleTemplate input[name=extra_class]').val();
                var template = $('#frmSectionAddonServiceModuleTemplate input[name=template]').val();
                var config = $('#frmSectionAddonServiceModuleTemplate select[name=section_config]').val();
                var configTxt = $('#frmSectionAddonServiceModuleTemplate .config option:selected').text();
                var layout = $('#frmSectionAddonServiceModuleTemplate select[name=layoutType]').val();
                ServiceModule.addonserviceTemplate(val, config, template, edit, configTxt, layout, extra_class);
                $('#sectionAddonServiceModuleTemplate').modal('hide');
            }
        },
        // 24-11-2021 Add On Services End Code
...
// 24-11-2021 Add On Services Start Code
var addonserviceTemplate = function () {
    var addonserviceTemplate = function () {
        $("#frmSectionAddonServiceModuleTemplate").validate({
            errorElement: 'span', //default input error message container
            errorClass: 'help-block', // default input error message class
            ignore: [],
            rules: {
            ...
            invalidHandler: function (event, validator) { //display error alert on form submit
                ...
                $('.alert-danger', $('#frmSectionAddonServiceModuleTemplate')).show();
            },
            highlight: function (element) { // hightlight error inputs
                $(element).closest('.form-group').addClass('has-error'); // set error class to the control group
            },
            ...
            submitHandler: function (form) {
                ServiceModule.submitFrmAddonSectionServiceModuleTemplate();
                return false;
            }
        });
        $('#frmSectionAddonServiceModuleTemplate input').keypress(function (e) {
            if (e.which == 13) {
                ServiceModule.submitFrmAddonSectionServiceModuleTemplate();
                return false;
            }
    ...
    return {
        //main function to initiate the module
        init: function () {
            addonserviceTemplate();
        },
        reset: function () {
            var validator = $("#frmSectionAddonServiceModuleTemplate").validate();
            validator.resetForm();
        }
    };
}();
...
// 24-11-2021 Add On Services Start Code
$(document).on('click', '.add-on-service', function (event) 
{
    $('#sectionAddonServiceModuleTemplate [data-dismiss="modal"]').attr( "data-toggle", "" );
    $('#sectionAddonServiceModuleTemplate [data-dismiss="modal"]').attr( "data-target", "" );

    $('#pgBuiderSections').modal('hide');
    $('#sectionAddonServiceModuleTemplate').modal({
        backdrop: 'static',
        keyboard: false
    });
    ...
    if (typeof id != 'undefined') 
    {
        ...
        $('#frmSectionAddonServiceModuleTemplate input[name=editing]').val(id);
        $('#frmSectionAddonServiceModuleTemplate input[name=section_title]').val($.trim(value));
        $('#frmSectionAddonServiceModuleTemplate select[name=layoutType] option[value=' + layout + ']').prop('selected', true);
        $('#frmSectionAddonServiceModuleTemplate .config option[value=' + config + ']').prop('selected', true);
        $('#frmSectionAddonServiceModuleTemplate input[name=extra_class]').val(extclass);
        $('#frmSectionAddonServiceModuleTemplate .addSection').text('Update');
        $('#frmSectionAddonServiceModuleTemplate #exampleModalLabel b').text('Edit Add On Service');

    } else {

        var value = $(this).text();
        $('#frmSectionAddonServiceModuleTemplate input[name=editing]').val('');
        $('#frmSectionAddonServiceModuleTemplate input[name=section_title]').val($.trim(value));
        //$('#frmSectionAddonServiceModuleTemplate select[name=layoutType] option:first').prop('selected', true);
        $('#frmSectionAddonServiceModuleTemplate input[name=extra_class]').val('');
        $('#frmSectionAddonServiceModuleTemplate .addSection').text('Add');
        $('#frmSectionAddonServiceModuleTemplate #exampleModalLabel b').text('Add Add On Service');

        $('#sectionAddonServiceModuleTemplate [data-dismiss="modal"]').attr( "data-toggle", "modal" );
//        $('#sectionAddonServiceModuleTemplate [data-dismiss="modal"]').attr( "data-target", "#pgBuiderSections" );

    }

    $('#frmSectionAddonServiceModuleTemplate').find('input[name=template]').val($(this).data('filter'));
...
// 24-11-2021 Add On Services Start Code
$('#sectionAddonServiceModuleTemplate').on('shown.bs.modal', function () {
    $('#sectionAddonServiceModuleTemplate select').selectpicker('');
        addonserviceTemplate.init();
    }).on('hidden.bs.modal', function () {
        $('#sectionAddonServiceModuleTemplate select').selectpicker('destroy');
        addonserviceTemplate.reset();
});
// 24-11-2021 Add On Services End Code
