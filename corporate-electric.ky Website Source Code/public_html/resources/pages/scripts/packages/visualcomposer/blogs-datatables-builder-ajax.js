...
$(document).on('click', '.feature-blogs', function (event) {
    // alert("kmdhfbdf");
    caption = $(this).text();
    template = $(this).data('filter');
    id = '';
    ignoreItems = '';
    $('#pgBuiderSections').modal('hide');
    $('#sectionFeatureBlogsModule').modal({
        backdrop: 'static',
        keyboard: false
    });
});

$(document).on('click', '.featureblogs-module', function (event) {
...
$('#sectionFeatureBlogsModule').on('shown.bs.modal', function () {
    caption = $.trim(caption);
    
    $('#datatable_blogs_ajax').closest('.col-md-12').loading('start');
    FeatureBlogsDataTable.resizeModuleModal();
    validateSectionFeatureBlogs.init();
    $(this).find('.group-checkable').prop('checked', false);
    selectedItems = [];
    ignoreItems = [];
    $('select').selectpicker('destroy');
    FeatureBlogsDataTable.getCategory();
    $('#frmSectionFeatureBlogsModule #blogscategory-id option:first').prop('selected', true);
    $('#frmSectionFeatureBlogsModule #columns option:selected').prop('selected', false);
    $('#frmSectionFeatureBlogsModule #columns option[value=dtDateTime]').prop('selected', true);
    $('#frmSectionFeatureBlogsModule #columns option[value=desc]').prop('selected', true);
    $('#frmSectionFeatureBlogsModule input[name=template]').val(template);
    if (template == 'featured-blogs') {
        $('#frmSectionFeatureBlogsModule select[name=layoutType] option[class=list]').hide();
    }
    var layout = '';
    if (id != '') {
        caption = $('#' + id).data('caption');
        ...
        desc = $('#' + id).data('desc');
        var config = $('#' + id).data('config');
        $('#frmSectionFeatureBlogsModule input[name=editing]').val(id);
        // $('#frmSectionFeatureBlogsModule select[name=layoutType] option[value=' + layout + ']').prop('selected', true);
        $('#frmSectionFeatureBlogsModule #config').children('option[value=' + config + ']').prop('selected', true);

        $('.section-item[data-editor=' + id + '] li').each(function (key, val) {
            var iId = $(this).data('id');
            ignoreItems.push(iId);
        });
        $('#sectionFeatureBlogsModule #addSection').text('Update');
        $('#sectionFeatureBlogsModule #exampleModalLabel b').text('Update Feature Blogs');
    } else {
        $('#frmSectionFeatureBlogsModule input[name=editing]').val('');
        $('#frmSectionFeatureBlogsModule select[name=layoutType] option:first').prop('selected', true);
        $('#frmSectionFeatureBlogsModule #config').children('option[value=7]').prop('selected', true);

        $('#sectionFeatureBlogsModule #addSection').text('Add');
        $('#sectionFeatureBlogsModule #exampleModalLabel b').text('Feature Blogs');
    }

    $('#frmSectionFeatureBlogsModule #section_title').val(caption);
    $('#frmSectionFeatureBlogsModule #extra_class').val(extraclass);
    $('#frmSectionFeatureBlogsModule #section_description').val(desc);
    $('select').selectpicker();

    FeatureBlogsDataTable.init(start, range);
    $("#frmSectionFeatureBlogsModule #mcscroll").mCustomScrollbar({
        axis: "y",
        theme: "dark",
        callbacks: {
            onTotalScroll: function () {
                if ($('input[name=found]').val() > 0) {
                    $('#datatable_blogs_ajax').closest('.col-md-12').loading('start');
                    setTimeout(function () {
                        if ($('#sectionFeatureBlogsModule #record-table tr').length < $('input[name=total_records]').val()) {
                            start += range;
                            end += range;
                            FeatureBlogsDataTable.init(start, range);
                        } else {
    ...
    $('#sectionFeatureBlogsModule select[name=layoutType] option[class=list]').show();
    $('#sectionFeatureBlogsModule #record-table').html('');
    $(".record-list").sortable().disableSelection();
    validateSectionFeatureBlogs.reset();
});

$(document).on('keyup', '#sectionFeatureBlogsModule #searchfilter', function () {
    range = 10;
    start = 0;
    end = range;
    $('#sectionFeatureBlogsModule #record-table').html('');
...
$(document).on('change', '#sectionFeatureBlogsModule #columns', function () {
    range = 10;
    start = 0;
    end = range;
    $('#sectionFeatureBlogsModule #record-table').html('');
...
$(document).on('change', '#sectionFeatureBlogsModule #blogscategory-id', function () {
    range = 10;
    start = 0;
    end = range;
    $('#sectionFeatureBlogsModule #record-table').html('');
...
//Group checkbox checking
$(document).on('change', '#sectionFeatureBlogsModule .group-checkable', function () {
   
    if ($(this).prop('checked')) {
        $('#sectionFeatureBlogsModule #record-table .chkChoose').prop('checked', true);
        $('#sectionFeatureBlogsModule #record-table .chkChoose').parent().parent().parent().addClass('selected-record');
        $('#sectionFeatureBlogsModule #record-table .chkChoose:checked').each(function (index, value) {
            selectedItems.push($(this).val());
        });
    } else {
        $('#sectionFeatureBlogsModule #record-table .chkChoose').prop('checked', false);
        $('#sectionFeatureBlogsModule #record-table .chkChoose').parent().parent().parent().removeClass('selected-record');
        selectedItems = [];
    }
});

$(document).on('change', '#sectionFeatureBlogsModule #record-table .chkChoose', function () {
    if ($(this).prop('checked')) {
        selectedItems.push($(this).val());
        $(this).parent().parent().parent().addClass('selected-record');
    } else {
    ...
    if ($('#sectionFeatureBlogsModule #record-table .chkChoose:checked').length == $('#sectionFeatureBlogsModule #record-table tr .chkChoose').length) {
        $('#sectionFeatureBlogsModule .group-checkable').prop('checked', true);
    } else {
        $('#sectionFeatureBlogsModule .group-checkable').prop('checked', false);
    }
});

$(document).on('click', '#sectionFeatureBlogsModule #record-table tr', function () {
    if ($(this).find('.chkChoose').prop('checked')) {
        $(this).find('.chkChoose').prop('checked', false);
        $(this).removeClass('selected-record');
    } else {
...
var FeatureBlogsDataTable = function () {
    // public functions
    return {
        //main function
        init: function (from, to) {
            var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
            var sort = $('#sectionFeatureBlogsModule #columns').val();
            var ajaxUrl = site_url + '/powerpanel/blogs/get_builder_list';
            ...
            jQuery.ajax({
                ...
                dataType: 'JSON',
                data: {
                    critaria: $('#frmSectionFeatureBlogsModule input[name=template]').val(),
                    columns: sort[0],
                    order: sort[1],
                    catValue: $('#sectionFeatureBlogsModule #blogscategory-id').val(),
                    status: '',
                    searchValue: $('#sectionFeatureBlogsModule #searchfilter').val(),
                    start: from,
                    length: to,
                    ignore: ignoreItems,
                    selected: selectedItems
                ...
                async: false,
                success: function (result) {
                    $('#sectionFeatureBlogsModule #record-table').append(result.data);
                    $('input[name=total_records]').val(result.recordsTotal);
                    $('input[name=found]').val(result.found);
                },
                error: function (req, err) {
                ...
                complete: function () {
                    if ($('#sectionFeatureBlogsModule #record-table tr').length > 1) {
                        $('#sectionFeatureBlogsModule #record-table #not-found').remove();
                    }
                    $('#datatable_blogs_ajax').closest('.col-md-12').loading('stop');
                }
            });
        ...
        getCategory: function (from, to) {
            ...
            jQuery.ajax({
                ...
                async: false,
                success: function (result) {
                    $('#sectionFeatureBlogsModule #blogscategory-id').html(result);
                },
                complete: function () {
        ...
        resizeModuleModal: function () {
            ...
            if ($.cookie('blogsModuleH')) {
                var blogsModuleH = $.cookie('blogsModuleH');
                $('#sectionFeatureBlogsModule .modal-dialog').css('height', blogsModuleH);
                $('#sectionFeatureBlogsModule .modal-content').css('height', blogsModuleH);
            }
            if ($.cookie('blogsModuleW')) {
                var blogsModuleW = $.cookie('blogsModuleW');
                $('#sectionFeatureBlogsModule .modal-dialog').css('width', blogsModuleW);
                $('#sectionFeatureBlogsModule .modal-content').css('width', blogsModuleW);
            }

            if ($.cookie('blogsTableH')) {
                var blogsTableH = $.cookie('blogsTableH');
                $('#sectionFeatureBlogsModule #mcscroll').css('height', blogsTableH);
            }

            $('#sectionFeatureBlogsModule .modal-content').resizable({
                alsoResize: ".modal-dialog,#mcscroll",
                minHeight: 607,
                minWidth: 750,
                maxHeight: 727,
