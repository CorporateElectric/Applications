...
//    });
    ...
    jQuery(function ($) {
        var input = $('#phone_number_request')
    ...
    $('input#phone_number_request').val(); //=> '+1 (415) 123-5554'
    $('input#phone_number_request').mobilePhoneNumber('validate'); //=> true

    /** start country code before added + sign code **/
    $("#phone_number_request").on("blur", function () {
        var mobNum1 = $(this).val();
        var newVal = mobNum1.replace(/\D/g, '');
        if (newVal.length >= 11) {
            $('input#phone_number_request').val('+' + newVal);
